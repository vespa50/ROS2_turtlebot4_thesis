...
while tline~=-1                             %while there are lines to read
    ...
    elseif index==5                         %index 5: contain y coordinate
        ...
        [x,y]=rotate(x,y,delta_a,0,0);      %rotation
        x=-x+delta_x;                       %x translation
        y=-y+delta_y;                       %y translation
...
elseif action=="dist"                               %show statistics of measurments
    ...
    plot_matrix_y=bins/(d_max/nbin*R2);                          %define y axis value based on bin
    trapz(plot_matrix_x,plot_matrix_y)
    ...
    ylabel('EPDF','Interpreter','latex');   %label y axis
    ...
    box on;
    hold on;                                        %hold on plotted points
    plot(plot_matrix_x,cdf,'b-','LineWidth',1);     %plot the cdf on graph
    legend({'rectangular trajectory','square trajectory'},'Location','southeast');% add legend when comparing experiments
    xlabel('$d$ [m]','Interpreter','latex');         %label x axis
    ylabel('ECDF','Interpreter','latex');  %label y axis
